...
\begin{figure}
	<<echo=FALSE,results='hide',cache=T>>=
		pca_lstages=tapply(alldata$Biomass,list(alldata$Nodes_df,alldata$Simnum,alldata$Exper),mean)
		pca_lstages=pca_lstages[c("Fish_ls_1","Fish_ls_2","Fish_ls_4"),,]
		scatter3D(pca_lstages[1,,], pca_lstages[2,,], pca_lstages[3,,], xlab="Offspring", ylab="Juveniles", zlab="Adults", main="Total Biomass For Each Lifestage", colkey=F, col=0)
		legend(0.1,.45,1:N_Exper,col=1:N_Exper,pch=1,title="Experiment")
		for (i in 1:N_Exper){
			scatter3D(pca_lstages[1,,i],pca_lstages[2,,i],pca_lstages[3,,i],col=i,add=T)
		}
	@
