...
\begin{figure}
	<<echo=FALSE,results='hide',cache=T>>=
		pca_groups=tapply(alldata$Biomass,list(alldata$Nodes_df,alldata$Simnum,alldata$Exper),mean)
		pca_groups=pca_groups[c("Fish_tot_df","inverts_tot_df","basal_tot_df"),,]
		scatter3D(pca_groups[1,,], pca_groups[2,,], pca_groups[3,,],xlab="Fish", ylab="Invertebrates", zlab="Autotrophs", main="Total Biomass For Each Group", colkey=F, col=0)
		legend(0.1,.45,1:N_Exper,col=1:N_Exper,pch=1,title="Experiment")
		for (i in 1:N_Exper){
			scatter3D(pca_groups[1,,i],pca_groups[2,,i],pca_groups[3,,i],col=i,add=T)
		}
	@
	\caption{A PCA plot where the dimensions represent different groups (fish, invertebrates, autotrophs).}
\end{figure}
\begin{figure}
	<<echo=FALSE,results='hide',cache=T>>=
