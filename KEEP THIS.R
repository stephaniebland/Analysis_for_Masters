...
VB_hist=subdat1 %>% filter(Exper==3,Year_df==max(Year_df)) %>%
	group_by(simnum) %>%
	mutate(scaled_mass=10^5*Mass/max(Mass)) %>%
	filter(lifestage==4)
...
mod1=lm(log_fish~max_Z,full_stats)
summary(mod1)
# SEND MOD2 TO ANNA
mod2=lm(CV_tot~max_Z,full_stats)
plot(mod2)
plot(full_stats$max_Z,full_stats$CV_tot)
summary(mod2)
