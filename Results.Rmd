...
The mean percent of extant nodes is shown in figure 1. Experiment 3 has a slightly higher percentage of extant nodes. This indicates that the new fish life stages we added are probably more likely to go extinct than an average node. (Although it could also be true that the new lifestages aren't more likely to go extinct, but the presence of them destabilizes other species instead so other species are more likely to go extinct when new nodes are added.)
...
There is no significant effect on the number of extant species (fig `r ex_spec`).
...
The mean number of extant species is shown in figure. Since we only require one life stage to persist for the duration of the simulation, fish will have a lower extinction rate in the experiments with additional life stages, which explains why experiment one and two have more persisting fish species.
...
Figure `r LH_Fratio` shows the fish to total species biomass ratio as a function of the two main life history traits: allometric ratio and adult fish aysmptotic body mass. There is a slight terned where larger fish that have a higher asymptotic body size will end up dominating more of the biomass. 
